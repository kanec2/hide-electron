...
class Main {
	...
	// В начале класса Ide, после объявления полей:
	private var windowBackend: IWindowBackend;

	public function new() {
		var args = process.argv;
		windowBackend = BackendFactory.create();
    	windowBackend.init();
		App.whenReady().then(_->{
			windowBackend.openWindow('app.html',{});
		});
